const productosApi = {
  get: () => {
    return fetch('http://localhost:8080/api/productos/').then((data) =>
      data.json()
    );
  },
  post: (nuevoProd) => {
    ...
    return fetch('http://localhost:8080/api/productos/', options);
  },
  put: (idProd, nuevoProd) => {
    ...
    return fetch(`http://localhost:8080/api/productos/${idProd}`, options);
  },
  delete: (idProd) => {
    const options = {
      method: 'DELETE',
...
function borrarProducto(idProd) {
  productosApi.delete(idProd).then(actualizarListaProductos);
}

function actualizarProducto(idProd) {
  const nuevoProd = leerProductoDelFormulario();
  productosApi.put(idProd, nuevoProd).then(actualizarListaProductos);
}
