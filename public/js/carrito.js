...
const carritosApi = {
  crearCarrito: () => {
    ...
    return fetch('http://localhost:8080/api/carritos', options).then((data) =>
      data.json()
    );
  },
  getIds: () => {
    return fetch('http://localhost:8080/api/carritos').then((data) =>
      data.json()
    );
  },
...
document.getElementById('btnCrearCarrito').addEventListener('click', () => {
  carritosApi.crearCarrito().then(({ id }) => {
...
function loadComboProductos() {
  return productosApi.get().then((productos) => {
...
function loadComboCarrito() {
  return carritosApi.getIds().then((ids) => {
    const combo = document.getElementById('comboCarritos');
    vaciarCombo(combo);
